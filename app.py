# app.py
# Flask API لتنقية مصطلحات نفسية/جنسية حسّاسة بصياغات مهنية، مع OpenAPI وSwagger.
# لماذا: خادم بسيط متوافق مع Render وGPT Actions (servers: "/")
...
# Regex عربي مرن (تجاهل التشكيل/المد + توحيد اختلافات الحروف الشائعة)
...
def sanitize_text_strict(text: str) -> str:
    """يستبدل المصطلحات الحساسة بصياغات مهنية باستخدام Regex مرن."""
    out = text
    for pattern, rep in _COMPILED_RULES:
        out = pattern.sub(rep, out)
    return out

# OpenAPI (servers نسبي لتلبية شرط same-origin في GPT Actions)
OPENAPI_SPEC = {
    "openapi": "3.1.0",
    "info": {
